...
{
    public class StatementFileRepository
    {
        private JsonFileRepository<Statement> _jsonFileRepository = new();

        public IEnumerable<Statement> GetStatements(string userPath, CardType? cardType = null, bool excludeBillPayment = false, bool excludeCorrelations = false, string where = null, string[] args = null, string orderby = null)
        {
            ...
            foreach (string d in directories)
            {
                ...
                foreach (var f in files)
                {
                    ...
                    var statement = _jsonFileRepository.GetFile(f);
        ...
        public IEnumerable<Transaction> GetTransactions(string userPath, string idOrName, string where = null, string[] args = null, string orderby = null)
        {
            var statements = GetStatements(userPath);
        ...
        public void Save(Statement e, string statementPath)
        {
            _jsonFileRepository.Save(e, statementPath);
            //FileExtensions.CreateFolderIfNeeded(statementPath);
            //File.WriteAllText(statementPath, JsonConvert.SerializeObject(e, Formatting.Indented));
        }
    }
}
